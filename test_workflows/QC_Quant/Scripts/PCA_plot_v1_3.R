...
PCA_Plot <- function(X, id, log_data = TRUE, log_base = 2,
                     ...
                     label = FALSE, PCx = 1, PCy = 2, label_size = 4, alpha = 1) {
 ...
 #   id3 <- id2[rowSums(is.na(X_2)) == 0,]
  ...
  text_message <- paste0(nrow(X_3), " of ", nrow(X), " rows are used for PCA.")
  ...
  text_message <- paste0(text_message, paste0(" 50% explained variance is reached with ", var50, " principle components."))
  ...
  if (!is.null(groupvar1) & !is.null(groupvar2)) {
    ...
    pl <- ggplot(data = D_PCA, aes(x=PCx, y=PCy)) +
      geom_point(aes(colour = groupvar1, shape = groupvar2), size = point.size, alpha = alpha)
  ...
  if (!is.null(groupvar1) & is.null(groupvar2)) {
    ...
    colnames(D_PCA)[1:2] <- c("PCx", "PCy")
    pl <- ggplot(data = D_PCA, aes(x=PCx, y=PCy)) +
      geom_point(aes(colour = groupvar1), size = point.size, alpha = alpha)
  ...
  if (returnPCA) {
    return(list(pl = pl, D_PCA_plot = cbind(D_PCA, Sample = colnames(X)), pca = pca, message = text_message))#,
               # D_PCA = cbind(id3, X_3)))
  } else {
    return(list(PCA = pl, message = text_message))
  }
