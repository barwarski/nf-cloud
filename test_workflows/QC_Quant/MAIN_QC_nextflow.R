...
library(ggrepel)
library(plotly)
library(rjson)     ### JSON-fles for Plotly-JSONs
...
ggplotly(ValidValue$pl_valid_values_rel)
ValidValue_JSON <- plotly_json(ggplotly(ValidValue$pl_valid_values_rel), FALSE)
write(ValidValue_JSON, paste0(output_path, "ValidValue.json"))
...
ggplotly(Boxplot)
write(plotly_json(ggplotly(Boxplot)), paste0(output_path, "Boxplot.json"))
...
PCA_plotly <- ggplotly(PCA_p$PCA)
write(plotly_json(ggplotly(PCA_p$PCA)), paste0(output_path, "PCA.json"))

PCA_messages <- NULL
PCA_messages <- PCA$message
...
### MA-Plots
MA_message <- NULL
MA_message <- MAPlots(X = D, log = FALSE, alpha = FALSE,
                      plot_height = plot_height_MAPlot, plot_width = plot_width_MAPlot,
                      output_path = output_path, suffix = suffix)

################################################################################
### Histrogramme
Histograms(X = D_long, plot_height = plot_height_hist, plot_width = plot_width_hist,
           group_colours = group_colours, suffix = suffix,
           #numcol = 2, numrow = 5, 
           xlimit = plot_xlim_hist, ylimit = plot_ylim_hist,
           num_bins = hist_numbins)

################################################################################
### Write File with Text Messages
writeLines(c( "Normalization:",Normalization_messages,"PCA:",PCA_messages, "\n","MA-Plots:", MA_message) ,sep = "\n", fileConn)
